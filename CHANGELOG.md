...
### Improvements
* Added `server.js` files for blueprints for convenient imports
...
### Improvements
* Renamed application entry point from `server.js` -> `app.js` (clarifies intent and helps with coverage reports)
...
* `reducers/server.js` now exports combined root reducer.
